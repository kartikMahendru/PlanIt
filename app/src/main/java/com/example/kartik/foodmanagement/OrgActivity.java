...
public class OrgActivity extends AppCompatActivity implements OnMapReadyCallback, PermissionsListener {
    ...
    private DatabaseReference myRef,foodRef,foodPickedRef;
    private LatLng orgStart;
    private Organization org;
    private double orgRange;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        iconFactory = IconFactory.getInstance(OrgActivity.this);
        ...
        myRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                try {
                    ...
                    Log.e("TAG","In range "+org.getName()+ " "+org.getAddress()+" "+org.getLatitude()+" "+org.getLongitude());
                    assert org != null;
                    orgStart = new LatLng(org.getLatitude(), org.getLongitude());
                    orgRange = org.getRange()*1000.0;
        ...
        foodRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for(DataSnapshot postSnapshot:dataSnapshot.getChildren()) {
                    ...
                    LatLng foodLatLng=new LatLng(post.getLatitude(),post.getLongitude());
                    if(foodLatLng.distanceTo(orgStart)<=orgRange) {
                        Log.e("TAG","In range "+postSnapshot.getKey());

                        // Optimization API is limited to 12 coordinate sets
                        if (alreadyTwelveMarkersOnMap()) {
                            Toast.makeText(OrgActivity.this, "Only 12 steps allowed", Toast.LENGTH_LONG).show();
                        } else {
                            Style style = mapboxMap.getStyle();
                            if (style != null) {
                                mapboxMap.addMarker(new MarkerOptions()
                                        .position(foodLatLng)
                                        .title("sufficient for "+(post.getSuffFor()))
                                        .snippet("\n"+post.getDescription()+"\nID: "+postSnapshot.getKey()))
                                        .setIcon(icon);
                                stops.add(Point.fromLngLat(foodLatLng.getLongitude(), foodLatLng.getLatitude()));
                                if(stops.size()>=2) {
        ...
        optimizedClient.enqueueCall(new Callback<OptimizationResponse>() {
            ...
            public void onResponse(@NotNull Call<OptimizationResponse> call, @NotNull Response<OptimizationResponse> response) {
                if (!response.isSuccessful()) {
                    ...
                    Toast.makeText(OrgActivity.this, "NO SUCCESS", Toast.LENGTH_SHORT).show();
                } else {
                    if (response.body() != null) {
                        ...
                        if (routes != null) {
                            if (routes.isEmpty()) {
                                ...
                                Toast.makeText(OrgActivity.this, "SUCCESSFUL BUT NO ROUTES",
...
//								}
                            ...
                            Toast.makeText(OrgActivity.this, String.format("NULL RESPONSE",
    ...
    public void openDialog(final Marker marker1) {
        ...
        alertDialog.setButton(AlertDialog.BUTTON_POSITIVE, " YES ", (dialogInterface, i) -> {
            ProgressDialog pd=new ProgressDialog(OrgActivity.this);  // To show progress dialog
            pd.setMessage("Picking...");
            pd.setCancelable(false);
            pd.show();
            String key1=marker1.getSnippet().substring(marker1.getSnippet().lastIndexOf(':')+2);
            foodRef.child(key1).addListenerForSingleValueEvent(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                    ...
                    foodPickedRef.addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot1) {
                            if(dataSnapshot1.child(usedate).exists()) {
                                foodPickedRef.child(usedate).setValue(dataSnapshot1.child(usedate).getValue(Integer.class)+1);
                                foodRef.child(key1).setValue(null);
                                showFoodPoints();
                                pd.cancel();
                            } else {
                                foodPickedRef.child(usedate).setValue(1);
                                foodRef.child(key1).setValue(null);
                                foodRef.child(key1).setValue(null);
                                showFoodPoints();
                                pd.cancel();
